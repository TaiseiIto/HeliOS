...
impl From<&[u32]> for Index2Address {
    fn from(registers: &[u32]) -> Self {
        let (index2address, index_and_low_memory_address): (
            BTreeMap<usize, Address>,
            Option<(usize, Memory)>,
        ) = registers.iter().cloned().enumerate().fold(
            (BTreeMap::new(), None),
            move |(mut index2address, index_and_low_memory_address), (index, register)| {
                match index_and_low_memory_address {
                    Some((index, low_memory_address)) => {
                        ...
                        let low_address: u32 =
                            low_memory_address.base_address() << Memory::BASE_ADDRESS_OFFSET;
                    ...
                    None => {
                        ...
                        let memory: Option<Memory> =
                            (!memory.memory_space_indicator()).then_some(memory);
                        ...
                        match (memory, io) {
                            (Some(memory), None) => match memory.size() {
                                Size::Bits32 => {
                                    let address: u32 =
                                        memory.base_address() << Memory::BASE_ADDRESS_OFFSET;
                            ...
                            (None, Some(io)) => {
                                ...
                                let address = Address::Io { address };
                                index2address.insert(index, address);
                                (index2address, None)
                            }
                            _ => unreachable!(),
...
impl fmt::Debug for Index2Address {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        let Self(index2address) = self;
        formatter.debug_map().entries(index2address.iter()).finish()
    }
}
...
pub enum Address {
    Io { address: u32 },
    Memory { address: u64, prefetchable: bool },
}
...
impl Address {
    ...
    pub fn read_vector<T>(&self, length: usize) -> Vec<T>
    where
        T: Default,
    {
        let size: usize = mem::size_of::<T>();
        (0..length)
            .map(|index| self.offset(index * size).read())
            .collect()
    }
}
