...
pub struct Table {
    descriptors: Vec<short::Descriptor>,
}
...
impl Table {
    ...
    pub fn selector2descriptor(&self) -> BTreeMap<Selector, short::Interface> {
        ...
        (0..self.descriptors.len())
            .filter_map(move |index| {
                ...
                if short_descriptor_indices.contains(&index) {
                    let descriptor: Option<short::Interface> = (&self.descriptors[index]).into();
                    ...
                    let descriptor: Option<short::Interface> = (&descriptor).into();
                    descriptor.map(|descriptor| (selector, descriptor))
                } else {
                    None
                }
    ...
    pub fn set_task_state_segment_descriptor(&mut self, task_state_segment_descriptor: &x64::task::state::segment::Descriptor) -> Selector {
        ...
        let higher_descriptor: short::Descriptor = higher_descriptor.into();
    ...
    fn free_descriptor_indices(&self) -> BTreeSet<usize> {
        self.descriptors
            ...
            .fold((BTreeSet::<usize>::new(), false), |(mut free_descriptor_indices, previous_descriptor_is_lower_of_long), (index, descriptor)| if previous_descriptor_is_lower_of_long {
                ...
                let interface: Option<short::Interface> = descriptor.into();
                match interface {
...
impl From<Register> for Table {
    fn from(register: Register) -> Self {
        let descriptors: &[short::Descriptor] = unsafe {
            slice::from_raw_parts(register.base(), register.length())
        };
        let descriptors: Vec<short::Descriptor> = (u16::MIN..=u16::MAX)
            ...
            .map(|segment_selector| *descriptors
                ...
                .unwrap_or(&short::Descriptor::default()))
            .collect();
        Self {
            descriptors,
        }
