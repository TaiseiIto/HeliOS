...
use {
    ...
    std::iter,
    syn::{
        parse, Attribute, Data, DataStruct, DeriveInput, Field, Fields, FieldsNamed, Ident, Meta,
        MetaList, Path, PathSegment,
    },
};
...
pub fn derive_offset_getter(input: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let repr_packed: bool = attrs.iter().any(|attribute| {
        let Attribute {
            pound_token: _,
            style: _,
            bracket_token: _,
    ...
    let getters: Vec<proc_macro2::TokenStream> = match data {
        Data::Struct(DataStruct {
            ...
            let previous_fields: Vec<Option<&Field>> = iter::once(None)
                .chain(fields[..fields.len() - 1].iter().map(|field| Some(*field)))
                .collect();
            fields
                .into_iter()
                .zip(previous_fields)
